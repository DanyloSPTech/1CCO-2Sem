...
public class Carrinho {
    ...
    public void deleteTodosProdutosPorCategoria(String categoria) {
        List <Produto> listaAtt = new ArrayList();
        for(Produto produto : produtos){
            if(!produto.getCategoria().equalsIgnoreCase(categoria)){
                listaAtt.add(produto);
            }
        }
        this.produtos = listaAtt;
    }
    ...
    public List<String> findNomesRepetidos() {
        ...
        for(Produto produto : produtos){
            for(Produto produto2 : produtos){
                if(produto.getNome().equalsIgnoreCase(produto2.getNome()) && produto.getId() != produto2.getId()){
